...
if (pinAnswer.pinNumber === myPin) {
    ...
    const operationAns = await inquirer.prompt([
        {
            message: chalk.bold.bgYellowBright("Choose any one operation you want to perform:"),
            name: "whatOperation",
            type: "list",
            choices: [
                "Check Balance",
    ...
    if (operationAns.whatOperation === "Check Balance") {
        console.log(chalk.bold.bgMagentaBright("Your current balance is: Rs.") +
            " " +
            currentBalance);
    }
        ...
        if (withDrawAmountAns.inputWithDrawAmount > currentBalance) {
            ...
            console.log(chalk.bold.green("Your balance after withdraw is now: ") +
                " " +
                currentBalance);
        }
    }
